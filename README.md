...
You can change the voice dynamically using ROS 2 parameters, without needing to restart the node or edit the code.

**Change the voice at runtime:**

```bash
ros2 param set /kyutai_tts_node voice "marius"
```

**Set the voice on launch:**

```bash
ros2 run ros2_kyutai_tts kyutai_tts_node --ros-args -p voice:="marius"
```
